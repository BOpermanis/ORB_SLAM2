    ...
    cv::Mat Map::GetMapCloud() {
        ...
        for (auto it: this->GetAllMapPoints()) {
            if (not it->isBad()) {
                ...
                out.push_back(it->GetReferenceKeyFrame());
            }
        }
        return out;
    }
        ...
        void Map::AssociatePlanesByBoundary(ORB_SLAM2::Frame &pF, bool out) {
            ...
            if (out)
                cout << "Plane associate in map  ID :  " << pF.mnId << "   num of Plane: " << pF.mnPlaneNum << " TH: "
                     << mfDisTh << endl;
            ...
            for (int i = 0; i < pF.mnPlaneNum; ++i) {
                ...
                for (set<MapPlane *>::iterator sit = mspMapPlanes.begin(), send = mspMapPlanes.end();
                     sit != send; sit++) {
        ...
        void Map::SearchMatchedPlanes(ORB_SLAM2::KeyFrame *pKF, cv::Mat Scw, const vector<ORB_SLAM2::MapPlane *> &vpPlanes,
            ...
            if (out)
                cout << "Plane associate in map  ID :  " << pKF->mnId << "   num of Plane: " << pKF->mnPlaneNum
                     << " TH: " << mfDisTh << endl;
